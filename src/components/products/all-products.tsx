...
function filterFn(
  ...
  filters: {
    ...
    publishStartDateFilter: Moment | null;
    publishEndDateFilter: Moment | null;
  }
): boolean {
  const {
    productTitleFilter,
  ...
  if (publishStartDateFilter || publishEndDateFilter) {
    ...
    const publishDate = moment(m.publish_date);
...
function handleProductDateFilterChange(value: RangeValue<Moment>) {
...
const AllProducts: FC = () => {
  ...
  useEffect(() => {
    if (!publishStartDateQuery) setPublishStartDateFilter(null);
    else setPublishStartDateFilter(moment(publishStartDateQuery));
  }, [publishStartDateQuery]);

  useEffect(() => {
    if (!publishEndDateQuery) setPublishEndDateFilter(null);
    else setPublishEndDateFilter(moment(publishEndDateQuery));
  }, [publishEndDateQuery]);
  ...
  const publishDateRange = useMemo<RangeValue<Moment>>(
